...
from analysis.DNNCluster.ClassificationDNNCluster import setupModel
...
def main():
    ...
    list_data_files = [DATASET_CONT, DATASET_0MM, DATASET_5MM]
    list_root_files = [ROOTFILE_CONT, ROOTFILE_0MM, ROOTFILE_5MM]
    ...
    for i, file in enumerate(list_data_files):
        ...
        npz_data = np.load(file + "_prediction.npz")
        ...
        for j, event in enumerate(root_parser.iterate_events(n=1000)):
        ...
        IRExport.export_CC6(ary_e=nn_pred[idx_clas_pos, 1],
                            ...
                            filename="CC6IR_NNPRED_CBRECO_" + list_data_files[i] + "_theta" + str(
                                threshold).replace(".", ""),
                            verbose=1,
                            veto=True)
