...
from src.SiFiCCNN.Plotter import PTSaliencyMap
...
def get_classifier_metrics(y_scores, y_true, theta=0.5, weighted=False):
    """
    Takes prediction and true labels and calculates True-Positive, False-Positive,
    True-Negative, False-Negative, accuracy, purity and efficiency for given sample
    ...
    Args:
        ...
        weighted (Boolean): If true, all metrics are weighted in calculation by class-weights
...
def write_metrics_classifier(y_scores, y_true):
    acc_base, eff_base, pur_base, conf_base = get_classifier_metrics(y_scores, y_true, theta=0.5)
    acc_weight, _, _, _ = get_classifier_metrics(y_scores, y_true, theta=0.5, weighted=True)
    ...
    acc_opt, eff_opt, pur_opt, conf_opt = get_classifier_metrics(y_scores, y_true, theta=theta)
    acc_opt_weight, _, _, _ = get_classifier_metrics(y_scores, y_true, theta=theta, weighted=True)
    print("\nMetrics base threshold: ")
