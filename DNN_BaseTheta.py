...
from models import DNN_base_regression_theta

tfmodel_regTheta = DNN_base_regression_theta.return_model(80)
neuralnetwork_regTheta = NeuralNetwork.NeuralNetwork(model=tfmodel_regTheta,
                                                     model_name=RUN_NAME,
                                                     model_tag=RUN_TAG + "_regtheta")
...
TrainingHandler.train_regTheta(neuralnetwork_regTheta,
                               data_cluster,
                               verbose=1,
                               epochs=epochs)
...
for i in range(len(NPZ_FILE_EVAL)):
    ...
    data_cluster = NPZParser.wrapper(dir_npz + NPZ_FILE_EVAL[i],
                                     set_testall=True,
                                     standardize=True)
    EvaluationHandler.eval_regression_theta(neuralnetwork_regTheta, DataCluster=data_cluster)