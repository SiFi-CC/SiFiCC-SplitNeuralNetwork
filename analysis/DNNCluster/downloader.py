...
def load(RootParser,
    ...
    for i, event in enumerate(RootParser.iterate_events(n=n)):
        ...
        for j, idx in enumerate(np.flip(idx_scatterer)):
            if j >= sx:
                break
            sample_features[i, j, :] = [event.RecoClusterEntries[idx],
        ...
        for j, idx in enumerate(np.flip(idx_absorber)):
            if j >= ax:
                break
            sample_features[i, (j + sx), :] = [event.RecoClusterEntries[idx],
                                               event.RecoClusterTimestamps_relative[idx],
                                               event.RecoClusterEnergies_values[idx],
                                               event.RecoClusterPosition[idx].x,
                                               event.RecoClusterPosition[idx].y,
        ...
        sample_labels[i] = event.compton_tag * 1
        sample_attributes[i, :] = np.array([event.target_energy_e,
        ...
        # write meta data
        sample_sp[i] = event.MCPosition_source.z
        sample_pe[i] = event.MCEnergy_Primary
