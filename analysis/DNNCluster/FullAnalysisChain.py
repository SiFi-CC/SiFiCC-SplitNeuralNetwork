...
def main():
    ...
    for file in [DATASET_0MM, DATASET_5MM]:
...
def main_temp():
    ...
    for file in [DATASET_0MM, DATASET_5MM]:
        ...
        # This is done this way cause y_scores gets a really dumb shape from tensorflow
        idx_clas_pos = np.zeros(shape=(data.entries,))
        for i in range(len(idx_clas_pos)):
            if data.labels[i] == 0:
                idx_clas_pos[i] = 1
        idx_clas_pos = idx_clas_pos == 1
        ...
        IRExport.export_CC6(ary_e=nn_pred[idx_clas_pos, 1],
                            ...
                            ary_py=nn_pred[idx_clas_pos, 7],
                            ary_pz=nn_pred[idx_clas_pos, 8],
                            filename="CC6IR_NNRECO_BACKGROUND_" + file + "_theta" + str(
                                threshold).replace(".", ""),
                            verbose=1,
                            veto=True)
