...
# --------------------------------------------------------------------------------------------------
# DEBUGGING PRINTS FOR MORE INFORMATION

def print_event_summary_debug(Rootdata, n=0):
    """Prints out a summary of one event"""
    ...
    # Interaction lists
    print("\nRealPosition_e | RealInteractions_e | DirectionScatter")
    for i in range(len(event.MCInteractions_e)):
        tmp_vec = event.MCPosition_e[i] - event.MCComptonPosition
        tmp_vec /= tmp_vec.mag
        print("({:7.3f}, {:7.3f}, {:7.3f}) | {:2} | ({:7.3f}, {:7.3f}, {:7.3f})".format(
            event.MCPosition_e.x[i],
            event.MCPosition_e.y[i],
            event.MCPosition_e.z[i],
    ...
    print("\nRealPosition_p | RealInteractions_p | DirectionScatter | Diff [rad]")
    for i in range(len(event.MCInteractions_p)):
        tmp_vec = event.MCPosition_p[i] - event.MCComptonPosition
        r = tmp_vec.mag
        tmp_vec /= tmp_vec.mag
        tmp_angle = event.calc_theta_dotvec(tmp_vec, event.MCDirection_scatter)
        print(
            "({:7.3f}, {:7.3f}, {:7.3f}) | {:2} | ({:7.3f}, {:7.3f}, {:7.3f}) | {:.5f} [rad] ({}) ({:7.5f} [mm])".format(
                event.MCPosition_p.x[i],
                event.MCPosition_p.y[i],
                event.MCPosition_p.z[i],
